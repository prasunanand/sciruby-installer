module SciRubyInstaller
  ...
  def self.unzip(package)
    ...
    Dir.chdir(SciRubyInstaller::Archives) do
      archive = File.join(SciRubyInstaller::Archives, filename)
      ...
      Zip::File.open(archive) do |zipfile|
        ...
        FileUtils.mkdir_p(File.join(SciRubyInstaller::Stage, "bin"))
        ...
        entries.each do |entry|
          ...
          if File.exist?(File.join(SciRubyInstaller::Archives, entry.name))
            FileUtils.mv(
              File.join(SciRubyInstaller::Archives, entry.name),
              File.join(SciRubyInstaller::Stage, "bin", entry.name),
              :force => true
            )
          end
        end
  ...
  def self.extract(package)
    Dir.chdir(SciRubyInstaller::Archives) do
      ...
      if package.target.nil?
        target_path = SciRubyInstaller::Stage
      else
        target_path = File.join(SciRubyInstaller::Stage, package.target)
      end
      bsdtar   = File.join(SciRubyInstaller::Stage, "bin", SciRubyInstaller::BSDTar.binary)
      sevenzip = File.join(SciRubyInstaller::Stage, "bin", SciRubyInstaller::SevenZip.binary)

      if package.category == "utility" &&
        File.exist?(File.join(SciRubyInstaller::Stage, "bin", package.binary))
      ...
      FileUtils.mkdir_p(SciRubyInstaller::Stage) unless File.directory?(SciRubyInstaller::Stage)

      case package.category
        when "utility" # Remove target file, if exists.
          target = File.join(SciRubyInstaller::Stage, "bin", package.binary)
        ...
        when "component" # Remove target dir if it exists and is different than the stage
          if (File.directory?(target_path) && target_path != SciRubyInstaller::Stage)
      ...
      Dir.chdir(SciRubyInstaller::Stage) do
          case filename
  ...
  def self.install_utility
    ...
    if File.exists?(File.join(SciRubyInstaller::Stage, "bin", binary))
      printf "#{File.join(SciRubyInstaller::Stage, "bin", binary)} exists.\nSkipping download, extract and install.\n"
    else
      ...
      Dir.chdir(SciRubyInstaller::Stage) do
        ...
        printf " => Installing #{binary} to #{File.join(SciRubyInstaller::Stage, "bin")}\n"

        FileUtils.mkdir_p(SciRubyInstaller::Stage, "bin") unless File.directory?(SciRubyInstaller::Stage, "bin")

        FileUtils.mv(
          File.join(SciRubyInstaller::Stage, binary),
          File.join(SciRubyInstaller::Stage, "bin", binary),
          :force => true
        )
      end
    end
  ...
  def self.stage_todo_application
    ...
    line = %Q(#{git_binary} clone -b SciRubyInstaller https://github.com/engineyard/todo todo)

    applications_path = File.join(SciRubyInstaller::Stage, "Sites")
  ...
  def self.stage_sciruby_sample_application
    # Generate sample sciruby application in the sciruby application directory on
    # stage.
    section Rails
  ...
  def self.stage_setup_scripts
    section Scripts
    scripts_path = File.join(SciRubyInstaller::Stage, "scripts")
    FileUtils.mkdir_p(scripts_path) unless File.exist?(scripts_path)

    %w( config_check.rb ).each do |file|
      FileUtils.cp(
        File.join(SciRubyInstaller::Scripts, file),
  ...
  def self.stage_msvc_runtime
    ...
    pkg_path = File.join(SciRubyInstaller::Stage, "pkg")

    FileUtils.mkdir_p(pkg_path) unless File.exist?(pkg_path)

    FileUtils.cp(
      File.join(SciRubyInstaller::Archives, File.basename(MsvcRuntime.url)),
  ...
  def self.sh(line, options = {})
    stage_bin_path = File.join(SciRubyInstaller::Stage, "bin")
