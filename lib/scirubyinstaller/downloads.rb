module SciRubyInstaller
  ...
  def self.download(url, filename = nil, count = 3)
   ...
   return if File.exists?(File.join(SciRubyInstaller::Archives, filename))
   ...
   begin
      ...
      print "Downloading from #{url} to #{SciRubyInstaller::Archives}/#{filename}\n" if $Flags[:verbose]
      http.get_response(uri) do |response|
        ...
        case response
          ...
          when Net::HTTPOK
            ...
            unless File.directory?(SciRubyInstaller::Archives)
              FileUtils.mkdir_p(SciRubyInstaller::Archives)
            end

            if File.exist?(File.join(SciRubyInstaller::Archives, filename))
              FileUtils.rm_f(File.join(SciRubyInstaller::Archives, filename))
            end
            ...
            Dir.chdir(SciRubyInstaller::Archives) do
              ...
              FileUtils.mv(
                temp_file.path,
                File.join(SciRubyInstaller::Archives, filename),
                :force => true
              )
      ...
      if File.exists?(File.join(SciRubyInstaller::Archives, filename))
        File.unlink(File.join(SciRubyInstaller::Archives, filename))
      end
