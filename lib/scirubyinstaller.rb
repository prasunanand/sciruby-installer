module SciRubyInstaller # Ensure that the SciRubyInstaller project root is defined.
...
%w[ globals ].each do |name|
  printf "Loading #{name}...\n" if $Flags[:verbose]
  require File.expand_path(File.join(SciRubyInstaller::Root, "lib", "setup", name + '.rb'))
end

%w[ stdlib ].each do |name|
  printf "Loading #{name}...\n" if $Flags[:verbose]
  require File.expand_path(File.join(SciRubyInstaller::Root, "lib", "extensions", name + '.rb'))
end

%w[ components methods downloads actions ].each do |name|
  printf "Loading #{name}...\n" if $Flags[:verbose]
  require File.expand_path(File.join(SciRubyInstaller::Root, "lib", "scirubyinstaller", name + '.rb'))
end
